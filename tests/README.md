...
2. **Migrate** — runs `AI` agent with the skill (migration, dummy, etc) against the project (output streams to console)
3. **Check** — runs verification checks (builds, tests pass, no Spring deps, has Quarkus, starts up) when defined part of the `project.yaml` file
